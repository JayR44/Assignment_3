...
#Import train data
subject_train <- read.table("UCI HAR Dataset/test/subject_train.txt", col.names = "subject_code")
X_train <- read.table("UCI HAR Dataset/test/X_train.txt", col.names = features$feature)
y_train <- read.table("UCI HAR Dataset/test/y_train.txt", col.names = "activity_code")
